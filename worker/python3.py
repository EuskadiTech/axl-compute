...
def on_message(client, userdata, msg):
    ...
    if ms["type"] == "python-inline":
        ...
        try:
          ...
          client.publish(f"axlcore/telemetry", safe_serialize({"type": ms["type"], "status": "Ok", "node": NODE}))
        except Exception as e:
          client.publish(ms["out-topic"], safe_serialize({"_err": str(e)}))
          client.publish(f"axlcore/telemetry", safe_serialize({"type": ms["type"], "status": str(e), "node": NODE}))
    elif ms["type"] == "cmd":
        try:
